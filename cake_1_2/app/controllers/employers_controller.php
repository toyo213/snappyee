		...
		if (!empty($this->data)) {
			...
			if(!empty($e)){
				...
				if (!$this->Employer->save($this->data)) {
					...
					//$this->Session->setFlash(__('Please fill out the T&C check box　利用規約に同意してください', true));
			...
			//			$this-> Employer -> save($employer);
			$this->flash('アカウントが作成されました　Your account has been successfully created.','http://job-tiger.com/employers/login');
			//pr($this->data);
			}
		}
